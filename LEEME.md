...
| La partida se queda parada del todo y nadie tiene ningún botón que pulsar | Esto era un bug: si al anfitrión le fallaba justo el paso de resolver una ronda (por ejemplo, un corte de red de un instante), la partida se quedaba congelada para siempre, aunque el fallo fuera pasajero. Ya está arreglado: ahora, si un paso falla, se reintenta solo en el siguiente segundo sin que nadie tenga que hacer nada. Si aun así se queda parada más de unos segundos, recarga la página del **anfitrión** (con la web abierta con el mismo código de sala, se reengancha sola donde estaba). |
| El navegador se queda "raro" entre partidas (hay que borrar el historial) | Cada partida cambia la dirección de la web para incluir su código (`?codigo=1234`), así cada partida tiene su propia URL en vez de compartir siempre la misma. Si alguna vez la pantalla se queda rara, prueba primero a quitar ese `?codigo=...` de la barra de direcciones y recargar (vuelve a la pantalla de inicio limpia) antes de recurrir a borrar todo el historial del navegador. |
